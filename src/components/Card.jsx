...
export default function Card({ card, columnId, isHighlighted }) {
  ...
  // When actively dragging, show a faded placeholder — NO transform, NO motion
  if (isDragging) {
    return (
      <div
        ref={setNodeRef}
        className="p-3 rounded-xl border-2 border-dashed border-drag-ph-border bg-drag-ph-bg h-[80px] opacity-30"
      />
    );
  }

  // Only apply transform during the pre-drag phase (before distance threshold met)
  ...
  return (
    ...
    >
      {isEditing ? (
        <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 px-2 py-1 text-xs font-medium text-white bg-brand hover:brightness-110 rounded-lg cursor-pointer"
            >
              Save
            </button>
            <button
              onClick={handleCancelEdit}
              className="px-2 py-1 text-xs font-medium text-txt-secondary bg-surface-input hover:bg-hover rounded-lg cursor-pointer"
            >
              Cancel
            </button>
          </div>
        ...
        <>
          <div className="flex items-start justify-between gap-2">
            <h4 className="text-sm font-semibold text-txt leading-snug flex-1">
              {title}
            </h4>
          ...
          {description && (
            <p className="mt-1.5 text-xs text-txt-secondary leading-relaxed line-clamp-2">
              {description}
            </p>
          )}
          <div className="mt-2.5 flex items-center gap-1.5">
            <span
              className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full ${priorityConfig.bgColor} ${priorityConfig.textColor}`}
            >
              <span
      ...
      {showConfirm && (
        <div className="absolute inset-0 bg-surface-overlay backdrop-blur-sm rounded-xl flex flex-col items-center justify-center gap-2 p-3 z-10">
          <p className="text-xs font-medium text-txt text-center">
            Delete this card?
          </p>
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 text-xs font-medium text-white bg-err hover:brightness-110 rounded-lg cursor-pointer transition-all"
            >
              Delete
            </button>
            <button
              ...
              className="px-3 py-1 text-xs font-medium text-txt-secondary bg-surface-input hover:bg-hover rounded-lg cursor-pointer transition-colors"
            >
              Cancel
            </button>
          </div>
