    ...
    <form onSubmit={handleSubmit}>
      <div className="p-3 bg-surface rounded-xl border border-edge shadow-sm space-y-3">
        <div>
          ...
          />
          {error && (
            <p className="text-xs text-err mt-1 ml-1">{error}</p>
          )}
        </div>
        ...
        <div className="flex gap-1.5">
          {Object.entries(PRIORITIES).map(([key, config]) => (
            <button
              ...
              style={
                priority === key
                  ? { '--tw-ring-offset-color': 'var(--card-surface)' }
                  : undefined
              }
            >
              {config.label}
        ...
        <div className="flex gap-2">
          <button
            type="submit"
            className="flex-1 px-3 py-2 text-sm font-medium text-white bg-brand hover:brightness-110 rounded-lg transition-all cursor-pointer"
          >
            Add Card
          </button>
          <button
            type="button"
            onClick={onCancel}
            className="px-3 py-2 text-sm font-medium text-txt-secondary bg-surface-input hover:bg-hover rounded-lg transition-colors cursor-pointer"
          >
            Cancel
          </button>
        </div>
